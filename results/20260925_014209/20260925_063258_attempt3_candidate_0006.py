...
def schedule_eval():
    ...
    # Programmatically discover and unroll all remaining CPU loops
    from exo.API_scheduling import unroll_loop
    from exo.core.LoopIR import LoopIR

    def find_first_unrollable(stmts, occurrences):
        for s in stmts:
            if isinstance(s, LoopIR.Seq) or type(s).__name__ == "Seq":
                name = str(s.iter)
                occurrences[name] = occurrences.get(name, 0) + 1
                if name != "p":
                    hi_val = getattr(s.hi, "val", None)
                    lo_val = getattr(s.lo, "val", None)
                    if isinstance(hi_val, int) and isinstance(lo_val, int):
                        bound = hi_val - lo_val
                        if bound <= 150:
                            return name, occurrences[name] - 1
                res = find_first_unrollable(s.body, occurrences)
                if res:
                    return res
            elif isinstance(s, LoopIR.If) or type(s).__name__ == "If":
                res = find_first_unrollable(s.body, occurrences)
                if res:
                    return res
                res = find_first_unrollable(s.orelse, occurrences)
                if res:
                    return res
        return None
    ...
    while True:
        occurrences = {}
        res = find_first_unrollable(gemmini._loopir_proc.body, occurrences)
        if res is None:
            break
        name, idx = res
        gemmini = unroll_loop(gemmini, gemmini.find_loop(name, occurrence=idx))
