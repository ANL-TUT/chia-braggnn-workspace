...
# EVOLVE-BLOCK-START
from exo.API_scheduling import unroll_loop, fuse, reorder_loops, set_memory
...
def schedule_eval():
    ...
    # Try to unroll CPU loops to reduce Rocket core overhead
    for name in ["i0", "i1", "i2", "j0", "j1", "j2", "i", "j", "k", "c", "h", "w", "row", "col", "ch", "orow", "ocol", "och", "krow", "kcol", "kch"]:
        while True:
            try:
                gemmini = unroll_loop(gemmini, gemmini.find_loop(name))
            except Exception:
                break
