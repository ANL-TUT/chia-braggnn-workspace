...
def schedule_eval():
    ...
    def find_and_unroll_one(proc, block_cursor):
            ...
            if type(stmt).__name__ == "ForCursor":
                ...
                if name != "p":
                    ...
                    if lo_val is not None and hi_val is not None:
                        ...
                        if 0 < trip_count <= 128:
                            try:
                                new_proc = unroll_loop(proc, stmt)
                                return new_proc, True
                            except Exception:
                                pass
                
                # Recurse into the body of the loop
                new_proc, success = find_and_unroll_one(proc, stmt.body())
                if success:
                    return new_proc, True
                    
            elif type(stmt).__name__ == "IfCursor":
                new_proc, success = find_and_unroll_one(proc, stmt.then_body())
                if success:
                    return new_proc, True
                if stmt.has_else():
                    new_proc, success = find_and_unroll_one(proc, stmt.else_body())
                    if success:
                        return new_proc, True
        return proc, False

    # Recursively unroll all small loops running on CPU
    while True:
        gemmini, success = find_and_unroll_one(gemmini, gemmini.body())
        if not success:
            break
