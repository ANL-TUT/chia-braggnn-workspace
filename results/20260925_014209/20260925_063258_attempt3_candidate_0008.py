...
def auto_unroll(proc):
    import exo.core.LoopIR as LoopIR
    ...
    while True:
        ...
        def visit(stmt):
            ...
            if isinstance(stmt, LoopIR.For):
                loop_name = str(stmt.iter)
                extent = stmt.extent
                if isinstance(extent, LoopIR.Const) and extent.value <= 64 and loop_name != 'p' and loop_name not in failed_loops:
                ...
                for s in stmt.then_body:
                    visit(s)
                for s in stmt.or_else:
                    visit(s)
                    
        for s in proc.value.body:
            visit(s)
        ...
        try:
            proc = unroll_loop(proc, proc.find_loop(target_loop))
        except Exception as e:
            failed_loops.add(target_loop)
