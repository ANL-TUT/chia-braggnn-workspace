...
def schedule_eval():
    ...
    # Auto-unroll all remaining CPU loops in post-order (innermost first)
    from exo.API_scheduling import SchedulingError, unroll_loop, simplify
    from exo.core.LoopIR import LoopIR

    def get_all_constant_loops_postorder(block):
        loops = []
        for stmt in block:
            if isinstance(stmt, LoopIR.For):
                loops.extend(get_all_constant_loops_postorder(stmt.body))
                if stmt.name != "p" and isinstance(stmt.hi, LoopIR.Const):
                    loops.append(stmt.name)
            elif isinstance(stmt, LoopIR.If):
                loops.extend(get_all_constant_loops_postorder(stmt.then_body))
                loops.extend(get_all_constant_loops_postorder(stmt.else_body))
        return loops

    failed_loops = set()
    while True:
        all_loops = get_all_constant_loops_postorder(gemmini._loopir_proc.body)
        candidate_loops = [l for l in all_loops if l not in failed_loops]
        if not candidate_loops:
            break

        target = candidate_loops[0]
        occurrence = 0
        while True:
            try:
                cursor = gemmini.find_loop(target, occurrence=occurrence)
            except (SchedulingError, Exception):
                failed_loops.add(target)
                break
            try:
                gemmini = unroll_loop(gemmini, cursor)
