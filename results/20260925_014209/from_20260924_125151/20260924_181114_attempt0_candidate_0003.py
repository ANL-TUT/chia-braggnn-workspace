...
nlb_qkv_conv_nofence = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, suffix="_nofence", fence=False)
nlb_qkv_conv_withfence = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, suffix="_withfence", fence=True)
...
def schedule_nlb():
    ...
    gemmini = call_eqv(gemmini, "nlb_qkv_conv_cpu(_) #0", nlb_qkv_conv_nofence)
    gemmini = call_eqv(gemmini, "nlb_qkv_conv_cpu(_) #0", nlb_qkv_conv_withfence)
...
def optimize_cpu_loops(gemmini):
        ...
        def traverse(block):
            ...
            for stmt in block:
                if type(stmt).__name__ == "ForSeqCursor":
                    loop_name = stmt.name()
                    if loop_name != "p" and loop_name not in skipped_loops:
                        target_loop = stmt
                        return
                    traverse(stmt.body())
                elif type(stmt).__name__ == "IfCursor":
                    traverse(stmt.then_body())
                    if stmt.has_else():
                        traverse(stmt.else_body())
        traverse(gemmini.body())
        if target_loop is None:
            break
        loop_name = target_loop.name()
        try:
            gemmini = unroll_loop(gemmini, target_loop)
            gemmini = simplify(gemmini)
        except Exception as e:
            skipped_loops.add(loop_name)
    return gemmini
