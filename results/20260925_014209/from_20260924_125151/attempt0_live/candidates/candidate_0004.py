...
nlb_qkv_conv_fence = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, fence=True)
...
def unroll_all_constant_loops(proc):
    def get_const_loop(stmts):
        for s in stmts:
            if s.__class__.__name__ == "Seq":
                if s.hi.__class__.__name__ == "Const":
                    if str(s.iter) != "p":
                        return str(s.iter)
                res = get_const_loop(s.body)
                if res:
                    return res
            elif s.__class__.__name__ == "If":
                res = get_const_loop(s.body)
                if res:
                    return res
                if s.orelse:
                    res = get_const_loop(s.orelse)
                    if res:
                        return res
        return None

    while True:
        loop_var = get_const_loop(proc._loopir_proc.body)
        if not loop_var:
            break
        try:
            loop_cursor = proc.find(f"for {loop_var} in _: _")
...
def schedule_eval():
    ...
    gemmini = unroll_all_constant_loops(gemmini)
    return gemmini
