...
def auto_unroll(proc):
    from exo.API_scheduling import unroll_loop

    blacklist = set()

    def find_first_unrollable(stmts):
        for s in stmts:
            if type(s).__name__ == "Seq":
                loop_var = str(s.iter)
                if loop_var != "p" and loop_var not in blacklist and type(s.hi).__name__ == "Const":
                    bound = s.hi.val
                    if bound <= 64:
                        return loop_var
                res = find_first_unrollable(s.body)
                if res:
                    return res
                ...
                if res:
                    return res
                res = find_first_unrollable(s.orelse)
                if res:
                    return res
        return None

    for _ in range(300):
        loop_var = find_first_unrollable(proc._loopir_proc.body)
        if not loop_var:
            break
        try:
            proc = unroll_loop(proc, loop_var)
        except Exception:
            blacklist.add(loop_var)

    return proc


braggnn_eval = auto_unroll(braggnn_eval)
# EVOLVE-BLOCK-END
