...
nlb_qkv_conv_nf = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, fence=False, suffix="_nf")
nlb_qkv_conv_f = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, fence=True, suffix="_f")
...
def schedule_nlb():
    ...
    gemmini = call_eqv(gemmini, "nlb_qkv_conv_cpu(_) #0", nlb_qkv_conv_nf)
    gemmini = call_eqv(gemmini, "nlb_qkv_conv_cpu(_) #0", nlb_qkv_conv_nf)
    gemmini = call_eqv(gemmini, "nlb_qkv_conv_cpu(_) #0", nlb_qkv_conv_f)
...
def inline_all(proc, pattern):
    while True:
        try:
            proc = inline(proc, pattern)
        except Exception:
            break
    return proc


def get_all_loops(block):
    loops = []
    for stmt in block:
        if type(stmt).__name__ == "For":
            loops.append(stmt)
            loops.extend(get_all_loops(stmt.body))
        elif type(stmt).__name__ == "If":
            loops.extend(get_all_loops(stmt.body))
            if stmt.orelse:
                loops.extend(get_all_loops(stmt.orelse))
    return loops
...
def schedule_eval():
    ...
    # Inline braggnn_inference and all nested scheduled layers to flatten the execution path
    gemmini = inline_all(gemmini, "braggnn_inference(_)")
    gemmini = inline_all(gemmini, "conv1(_)")
    gemmini = inline_all(gemmini, "nlb(_)")
    gemmini = inline_all(gemmini, "conv2(_)")
    gemmini = inline_all(gemmini, "conv3(_)")
    gemmini = inline_all(gemmini, "fc1(_)")
    gemmini = inline_all(gemmini, "fc2(_)")
    gemmini = inline_all(gemmini, "fc3(_)")
    gemmini = inline_all(gemmini, "fc4(_)")
    gemmini = inline_all(gemmini, "fc_output(_)")
    
    # Inline all sub-layers in NLB
    gemmini = inline_all(gemmini, "nlb_qkv_conv_nf(_)")
    gemmini = inline_all(gemmini, "nlb_qkv_conv_f(_)")
    gemmini = inline_all(gemmini, "matmul_theta_phi(_)")
    gemmini = inline_all(gemmini, "nlb_softmax(_)")
    gemmini = inline_all(gemmini, "matmul_attention_g(_)")
    gemmini = inline_all(gemmini, "nlb_out_conv(_)")
    gemmini = inline_all(gemmini, "resadd_relu(_)")
    
    # Unroll all constant-bound CPU loops inside braggnn_eval
    unrolled_something = True
    attempts = 0
    while unrolled_something and attempts < 1000:
        attempts += 1
        unrolled_something = False
        all_loops = get_all_loops(gemmini.body())
        for l in all_loops:
            loop_name = str(l.name)
            if loop_name != "p":
                try:
                    gemmini = unroll_loop(gemmini, gemmini.find_loop(loop_name))
                    unrolled_something = True
                    break
                except Exception:
                    pass
