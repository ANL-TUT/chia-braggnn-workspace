...
def optimize_cpu_loops(proc):
    ...
    while True:
        ...
        for name in loops:
            try:
                loop_cursor = proc.find_loop(name)
...
def schedule_eval():
    ...
    # Fully inline the entire inference hierarchy to eliminate function call overhead
    for name in ["braggnn_inference", "nlb", "conv1", "conv2", "conv3", "fc1", "fc2", "fc3", "fc4", "fc_output",
                 "nlb_qkv_conv_nf", "nlb_qkv_conv_f", "matmul_theta_phi", "nlb_softmax", "matmul_attention_g",
                 "nlb_out_conv", "resadd_relu"]:
        while True:
            try:
    ...
    # Optimize any remaining CPU loops (quantization, transpose, flatten)
