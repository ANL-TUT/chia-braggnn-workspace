...
def schedule_braggnn():
    ...
    for _ in range(2):
        try:
            gemmini = inline(gemmini, "nlb_qkv_conv_nf(_)")
        except Exception:
            pass
    try:
        gemmini = inline(gemmini, "nlb_qkv_conv_f(_)")
    except Exception:
        pass
    try:
...
def auto_unroll_loops(proc):
    ...
    def is_innermost_const_loop(stmt):
        if type(stmt).__name__ != 'For':
            return False
        try:
            hi_val = int(str(stmt.hi).strip())
            lo_val = int(str(stmt.lo).strip())
            bound = hi_val - lo_val
            if bound > 200 or bound <= 0:
                return False
        except ValueError:
            return False
        ...
        def has_loops(stmts):
            for s in stmts:
                ...
                if type(s).__name__ == 'If':
                    if has_loops(s.body) or (s.orelse and has_loops(s.orelse)):
                        return True
            return False
        if has_loops(stmt.body):
            return False
        return True

    def find_innermost_const_loops(stmts):
        res = []
        for s in stmts:
            if is_innermost_const_loop(s):
                res.append(s)
            elif type(s).__name__ == 'For':
                res.extend(find_innermost_const_loops(s.body))
            elif type(s).__name__ == 'If':
                res.extend(find_innermost_const_loops(s.body))
                if s.orelse:
                    res.extend(find_innermost_const_loops(s.orelse))
        return res

    failed_loops = set()
    while True:
        try:
            stmts = proc._loopir_proc.body
        except AttributeError:
            break
        candidates = find_innermost_const_loops(stmts)
        candidates = [c for c in candidates if c.name not in failed_loops]
        if not candidates:
            break
        cand = candidates[0]
        v = str(cand.name)
        try:
            loop_cur = proc.find_loop(v)
            proc = unroll_loop(proc, loop_cur)
        except Exception:
            failed_loops.add(v)
