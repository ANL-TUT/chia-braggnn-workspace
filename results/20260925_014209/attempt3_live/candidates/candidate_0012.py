...
def schedule_eval():
    ...
    def find_unrollable_innermost_loop(proc, block):
        for stmt in block:
            if type(stmt).__name__ == "ForCursor":
                # Inside-out traversal: first try to find an unrollable loop in the body
                inner = find_unrollable_innermost_loop(proc, stmt.body())
                if inner is not None:
                    return inner
                
                # If there are no loops inside, check if this loop itself can be unrolled
                if stmt.name() != "p":
                    lo_val = None
                    hi_val = None
                    try:
                        if type(stmt.lo()).__name__ == "LiteralCursor":
                            lo_val = stmt.lo().value()
                        if type(stmt.hi()).__name__ == "LiteralCursor":
                            hi_val = stmt.hi().value()
                    except Exception:
                        pass
                    
                    if lo_val is not None and hi_val is not None:
                        trip_count = hi_val - lo_val
                        if 0 < trip_count <= 128:
                            return stmt
                            
            elif type(stmt).__name__ == "IfCursor":
                inner = find_unrollable_innermost_loop(proc, stmt.then_body())
                if inner is not None:
                    return inner
                if stmt.has_else():
                    inner = find_unrollable_innermost_loop(proc, stmt.else_body())
                    if inner is not None:
                        return inner
        return None

    # Unroll all small loops running on CPU from the inside out
    while True:
        loop_to_unroll = find_unrollable_innermost_loop(gemmini, gemmini.body())
        if loop_to_unroll is None:
            break
        try:
            gemmini = unroll_loop(gemmini, loop_to_unroll)
        except Exception:
            break
