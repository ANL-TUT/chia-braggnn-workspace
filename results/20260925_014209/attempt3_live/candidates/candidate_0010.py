...
def auto_unroll(proc):
    try:
        def find_loops(stmt):
            ...
            loops = []
            if s_type == 'For':
                hi_type = type(stmt.hi).__name__
                hi_val = None
                if hi_type == 'Const':
                    hi_val = stmt.hi.val
                loops.append((stmt.name, hi_val))
                for s in stmt.body:
                    loops.extend(find_loops(s))
            elif s_type == 'If':
                for s in stmt.body:
                    loops.extend(find_loops(s))
        ...
        loops = []
        for s in proc._loopir_proc.body:
            loops.extend(find_loops(s))
            
        loop_names = []
        for name, val in loops:
            if name != 'p' and val is not None and val <= 16:
                if name not in loop_names:
                    loop_names.append(name)
                    
        for name in loop_names:
            try:
                proc = unroll_loop(proc, proc.find_loop(name))
            except Exception:
                pass
    except Exception:
        pass
