        ...
        def traverse(node):
            if isinstance(node, list):
                for x in node:
                    ...
                    if res:
                        return res
            elif type(node).__name__ == 'Seq':
                hi_type = type(node.hi).__name__
                if hi_type in ('Const', 'Literal'):
                    return str(node.iter)
                res = traverse(node.body)
                if res:
                    return res
            elif hasattr(node, '_fields'):
                for f in node._fields:
                    res = traverse(getattr(node, f))
                    if res:
                        return res
            return None

        return traverse(proc._impl)

    failed_loops = set()
    while True:
        loop_var = find_constant_loop(gemmini)
        if loop_var is None or loop_var in failed_loops:
            break
        try:
            gemmini = unroll_loop(gemmini, gemmini.find_loop(loop_var))
        except Exception:
            failed_loops.add(loop_var)
