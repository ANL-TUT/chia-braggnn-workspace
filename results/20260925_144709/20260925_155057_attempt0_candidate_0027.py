...
nlb_qkv_conv_fence = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, fence=True, suffix="_fence")
...
def auto_unroll(proc, max_val=64):
    import exo.core.LoopIR as LoopIR
    
    def find_unrollable(block, excluded):
        for stmt in block:
            if type(stmt).__name__ == "ForSeq":
                hi_type = type(stmt.hi).__name__
                if hi_type == "Const" and stmt.iter not in excluded:
                    val = getattr(stmt.hi, "val", getattr(stmt.hi, "value", None))
                    if isinstance(val, int) and val <= max_val:
                        return stmt.iter
                res = find_unrollable(stmt.body, excluded)
                if res:
                    return res
                ...
                if res:
                    return res
                res = find_unrollable(stmt.orelse, excluded)
                if res:
                    return res
        return None
    ...
    excluded = set()
    while True:
        loop_var = find_unrollable(p._loopir_proc.body, excluded)
        if loop_var is None:
            break
        try:
            p = unroll_loop(p, p.find_loop(loop_var))
        except Exception as e:
            excluded.add(loop_var)
    return p
