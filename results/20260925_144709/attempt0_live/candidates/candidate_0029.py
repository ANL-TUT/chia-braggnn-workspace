...
def sched_resadd(cpu):
    ...
    gemmini = divide_loop(gemmini, "i1", NLB_ROW_TILE, ["i1_o", "i1_i"], tail="cut")
    gemmini = simplify(gemmini)
    gemmini = replace(gemmini, "for i1_i in _:_", do_resadd)
    gemmini = replace(gemmini, gemmini.find_loop("i1_o").next(), do_resadd)
    gemmini = fence_after(gemmini, f"do_{name}(_) #1")
...
nlb_qkv_conv_nofence = sched_conv(nlb_qkv_conv_cpu, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS, 1, nofence=True)
