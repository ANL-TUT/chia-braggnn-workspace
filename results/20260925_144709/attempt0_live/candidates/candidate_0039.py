...
from exo.API_scheduling import unroll_loop, divide_loop
        ...
        def walk(stmts):
            for s in stmts:
                if type(s).__name__ == "For":
                    ...
                    def check_call(body):
                        ...
                        for sub_s in body:
                            if type(sub_s).__name__ == "Call":
                                ...
                                sub_body = sub_s.body() if callable(sub_s.body) else sub_s.body
                                check_call(sub_body)
                            elif type(sub_s).__name__ == "If":
                                sub_body = sub_s.body() if callable(sub_s.body) else sub_s.body
                                sub_orelse = sub_s.orelse() if callable(sub_s.orelse) else sub_s.orelse
                                check_call(sub_body)
                                check_call(sub_orelse)
                    body = s.body() if callable(s.body) else s.body
                    check_call(body)
                    ...
                    body = s.body() if callable(s.body) else s.body
                    orelse = s.orelse() if callable(s.orelse) else s.orelse
                    walk(body)
                    walk(orelse)
        walk(proc.body() if callable(proc.body) else proc.body)
        return loops
