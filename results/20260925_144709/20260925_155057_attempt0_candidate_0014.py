...
try:
    from braggnn_reference import flatten_fc1_cpu
except ImportError:
    flatten_fc1_cpu = None

try:
    from braggnn_reference import input_quantize_cpu
except ImportError:
    input_quantize_cpu = None


if flatten_fc1_cpu is not None:
    def sched_flatten(cpu):
        p = rename(cpu, "flatten_fc1")
        for b in ["i2", "i1", "i0", "k", "j", "i"]:
            try:
                p = unroll_loop(p, b)
            except Exception:
                pass
        return p
    flatten_fc1 = sched_flatten(flatten_fc1_cpu)


if input_quantize_cpu is not None:
    def sched_quantize(cpu):
        p = rename(cpu, "input_quantize")
        for b in ["i2", "i1", "i0", "k", "j", "i"]:
            try:
                p = unroll_loop(p, b)
            except Exception:
                pass
        return p
    input_quantize = sched_quantize(input_quantize_cpu)
...
def schedule_braggnn():
    ...
    if input_quantize_cpu is not None:
        gemmini = call_eqv(gemmini, "input_quantize_cpu(_)", input_quantize)
    ...
    if flatten_fc1_cpu is not None:
        gemmini = call_eqv(gemmini, "flatten_fc1_cpu(_)", flatten_fc1)
