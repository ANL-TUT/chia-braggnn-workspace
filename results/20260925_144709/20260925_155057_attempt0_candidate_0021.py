...
from exo.API_scheduling import rearrange_dim, reorder_loops
...
fc1_hwc = make_loop_matmul_fc_hwc("do_fc1_hwc", CONV3_FILTERS, CONV3_DIM, CONV3_DIM, FC1_UNITS, act=True)
...
def schedule_eval():
    ...
    try:
        gemmini = inline(gemmini, "flatten_fc1_cpu(_)")
        gemmini = inline(gemmini, "fc1_cpu(_)")
        gemmini = rearrange_dim(gemmini, "fc1_weights_ : _", [0, 2, 3, 1])
        gemmini = rearrange_dim(gemmini, "flatten_fc1_out : _", [1, 2, 0])
        gemmini = reorder_loops(gemmini, "kch krow")
        gemmini = reorder_loops(gemmini, "kch kcol")
        gemmini = replace(gemmini, gemmini.find("for j in seq(0, FC1_UNITS): _"), fc1_hwc)
        gemmini = fence_after(gemmini, "do_fc1_hwc(_)")
